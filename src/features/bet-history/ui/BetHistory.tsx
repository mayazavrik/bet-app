...
import { Card, Typography, Table, Tag, Button, Space, Tooltip, Empty } from "antd";
import {
	HistoryOutlined,
	CheckCircleOutlined,
	CloseCircleOutlined,
...
const BetHistory: React.FC = () => {
	...
	const filteredBets = bets.filter((bet) => {
		...
		if (
			filters.result !== null &&
			(bet.status !== BetStatus.COMPLETED || bet.result?.win !== filters.result)
		) {
			return false;
		}
		return true;
				...
				<Space>
					...
					{(filters.status || filters.result !== null) && (
						<Tooltip title="Сбросить фильтры">
							<Button icon={<DeleteOutlined />} onClick={clearFilters} size="small" danger />
						</Tooltip>
					)}
				</Space>
			}
