...
// Исправленный slice для работы с рекомендуемыми ставками
import { createSlice, PayloadAction } from '@reduxjs/toolkit';
import { Bet, BetStatus } from '@/shared/types/common';

// Интерфейс для рекомендуемой ставки
...
interface BetState {
  recommendedBet: RecommendedBet | null; // Изменено на объект
  currentBet: number | null;
  currentBetId: string | null;
  bets: Bet[];
  loading: boolean;
...
export const betSlice = createSlice({
  ...
  reducers: {
    ...
    // Изменяем тип payload на RecommendedBet
    fetchRecommendedBetSuccess: (state, action: PayloadAction<RecommendedBet>) => {
      state.recommendedBet = action.payload;
      state.loading = false;
    },
    fetchRecommendedBetFailure: (state, action: PayloadAction<string>) => {
